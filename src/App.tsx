...
export const App: React.FunctionComponent = () => {
  ...
  return (
    <div className="App">
      <header
        style={{ height: "54px", borderBottom: "1px", borderColor: "black" }}
      >
      ...
      {selected != null && (
        <div className={"App-Margin"}>
          ...
          {group[selected].members.map((m, i) => (
            <Member
              Member={m}
              key={i}
              Update={() => Load(setUser, setGroup, setSelected)}
            />
          ))}
        </div>
      )}
...
function mapJSONGroups(data: any, i: number): IGroup {
  return {
    ...
    open: data[i]["open"],
    total: data[i]["total"],
    members: data[i]["members"].map((item: any) => {
      return {
        id: item["id"],
...
function getGroupsByUser(u: IUser) {
  return fetch("http://localhost:8080/api/group/" + u.id, {
    method: "GET",
    mode: "cors",
    headers: {
      Accept: "application/json",
